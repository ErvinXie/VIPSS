...
double RBF_Core::Dist_Function(const double x, const double y, const double z) {
    return RBF_Core::Dist_Function(R3Pt(x, y, z));
}
...
void RBF_Core::Print_Record() {
    ...
    if (record_partition.size() == 0) {
        for (int i = 0; i < record_method.size(); ++i) {
            cout << record_method[i] << "\t\t" << record_kernal[i] << "\t\t" << record_energy[i] << "\t\t"
                 << record_time[i] << endl;
        }
        for (int i = 0; i < setup_timev.size(); ++i) {
            cout << setup_timev[i] << "\t\t" << init_timev[i] << "\t\t" << solve_timev[i] << "\t\t" << callfunc_timev[i]
                 << "\t\t" << invM_timev[i] << "\t\t" << setK_timev[i] << endl;
        }
    } else {
        for (int j = 0; j < record_partition.size(); ++j) {
            ...
            for (int i = j == 0 ? 0 : record_partition[j - 1]; i < record_partition[j]; ++i) {
                cout << setup_timev[i] << "\t\t" << init_timev[i] << "\t\t" << solve_timev[i] << "\t\t"
                     << callfunc_timev[i] << endl;
            }
        }
    }
...
void RBF_Core::Print_TimerRecord(string fname) {
    ...
    if (!fout.fail()) {
        for (int i = 0; i < setup_timev.size(); ++i) {
            fout << npoints[i] << '\t' << setup_timev[i] << "\t" << init_timev[i] << "\t" << solve_timev[i] << "\t"
                 << callfunc_timev[i] << "\t" << invM_timev[i] << "\t" << setK_timev[i] << endl;
        }
    }
    fout.close();
