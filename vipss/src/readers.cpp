...
bool readPLYFile(string filename, vector<double> &vertices, vector<double> &vertices_normal) {
    ...
    auto readVerticesAndNormal = [&vertices, &vertices_normal](stringstream &strs) {
        ...
        for (int i = 0; i < 3; ++i) {
            strs >> dvalue;
            vertices_normal.push_back(dvalue);
        }
    };
    ...
    while (getline(fin, oneline)) {
        ...
        if (isstart) {
            readVerticesAndNormal(strs);
            continue;
        } else {
            strs >> prefix;
            if (prefix == "end_header") {
                isstart = true;
                continue;
            }
        }
...
bool readSurfFile(string filename, vector<double> &vertices, vector<unsigned int> &faces2vertices,
    ...
    auto readVerticesField = [&vertices_field](stringstream &strs) {
        ...
        for (int i = 0; i < 3; ++i) {
            strs >> dvalue;
            vertices_field.push_back(dvalue);
        }
    };
...
bool readCurfFile(string filename, vector<double> &vertices, vector<unsigned int> &edges2vertices,
    ...
    auto readVerticesTangent = [&vertices_tangent](stringstream &strs) {
        ...
        for (int i = 0; i < 3; ++i) {
            strs >> dvalue;
            vertices_tangent.push_back(dvalue);
        }
    };
    ...
    while (getline(fin, oneline)) {
        ...
        if (prefix == "vn") {
            readVerticesTangent(strs);
            continue;
        } // vertex normal(surface/volume)/tangent(curve)/LookAt Vector(Curve)
        if (prefix == "vf") {
            readVerticesField(strs);
            continue;
        } // vertices field
...
bool readVolfFile(string filename, vector<double> &vertices, vector<unsigned int> &tets2vertices,
    ...
    auto readTets = [&tets2vertices](stringstream &strs) {
        ...
        for (int i = 0; i < 4; ++i) {
            strs >> ivalue;
            tets2vertices.push_back(ivalue - 1);
        }
    };
    auto readVerticesField = [&vertices_field](stringstream &strs) {
        double dvalue;
...
bool writeCurNetFile(string filename, const vector<double> &vertices, const vector<vector<int> > &edge2vertices,
    ...
    for (int i = 0; i < n_vertices; ++i) {
        auto p_v = p_vd + i * 3;
        auto p_vn = p_vnd + i * 3;
        outer << "v " << p_v[0] << " " << p_v[1] << " " << p_v[2] << " " << p_vn[0] << " " << p_vn[1] << " " << p_vn[2]
              << endl;
    }
